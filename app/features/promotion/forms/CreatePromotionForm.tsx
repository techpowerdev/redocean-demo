...
// import { createPromotion, getPromotions } from "@/services/promotionServices";
...
//     try {
//       const promotionResult = await createPromotion(PromotionFormData);
...
import { createPromotionActivity } from "@/services/promotionActivityServices";
...
const PromotionFormSchema = z.object({
  ...
  imageIds: z.array(z.string()).min(1, "อัปโหลดอย่างน้อย 1 รูป"),
});
...
export default function CreatePromotionForm() {
  ...
  async function onSubmit(data: PromotionFormValues) {
    try {
      ...
      const promotionResult = await createPromotion(promotionData);
      ...
      if (promotionResult) {
        ...
        await createPromotionActivity(promotionActivityData);
  ...
  useEffect(() => {
    ...
    form.setValue("imageIds", validImageIds);
    form.trigger("imageIds");
  }, [form, images]);
