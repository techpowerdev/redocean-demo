...
import { createUserOrder } from "@/services/orderServices";

type CreateOderItemType = {
  sku: string;
  quantity: number;
  unitPrice: number;
  discount: number;
  ...
  const createOrder = async () => {
    try {
      ...
      if (cart) {
        ...
        // ส่ง createOrderData ไปยัง API
        if (data.isStockSufficient) {
          // 2.process checkout (generate QR code)
          ...
          // const result = await createUserOrder(token || "", createOrderData);
