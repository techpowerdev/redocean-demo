...
import { UserFormSchema } from "@/zod-schemas/userSchema";
...
export function EditProfileForm({ user }: Props) {
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(UserFormSchema), // validate data with the schema
    defaultValues: {
  ...
  return (
    <Form {...form}>
      <h1 className="font-semibold text-lg">แก้ไขข้อมูลส่วนตัว</h1>
      <form
        onSubmit={form.handleSubmit(onSubmit)}
        className="flex flex-col gap-4 my-2"
      >
