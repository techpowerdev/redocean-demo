...
export function EditPromotion({ promotion }: Props) {
  // global state
  ...
  const form = useForm<EventFormValues>({
    ...
    defaultValues: {
      ...
      // discountGroupAmount:promotion?.promotionActivities?.[0]?.discountGroupAmount || 0,
      images: [],
    },
  });
  ...
  useEffect(() => {
    if (promotion) {
      form.reset({
        ...
        endAt: promotion?.endAt
          ? formatDateToDatetimeLocal(promotion.endAt)
          : "",
        discountType:
          (promotion?.promotionActivities?.[0]?.discountType as DiscountType) ||
          "fixed",
        discountAmount:
          promotion?.promotionActivities?.[0]?.discountAmount || 0,
        limitQuantity: promotion?.promotionActivities?.[0]?.limitQuantity,
        maxQuantity: promotion?.promotionActivities?.[0]?.maxQuantity || 0,
        limitQuantityPerUser:
          promotion?.promotionActivities?.[0]?.limitQuantityPerUser,
    ...
    setSelectedProduct(promotion?.promotionActivities?.[0].product || null);
  }, []);
        ...
        <Tabs defaultValue="event" className="w-full">
          ...
          <TabsContent value="eventDetail" className="space-y-6 px-2">
            ...
            {/* <SearchProductInEventForm
              initialProduct={promotion?.promotionActivities?.[0].product}
            /> */}
